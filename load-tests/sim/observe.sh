...
# Orphan/stale rate on node 0 (live snapshot). Count DISTINCT produced blocks
# vs the canonical chain's net growth -- NOT log-line counts: promote_block
...
if [ -f "$n0" ]; then
  ...
  rate="n/a"; mainchain=0; orphans=0
  if [ -n "${chain_hi:-}" ] && [ "$produced" -gt 0 ]; then
    mainchain=$(( chain_hi - chain_lo + 1 ))
    orphans=$(( produced - mainchain )); [ "$orphans" -lt 0 ] && orphans=0
    rate=$(awk "BEGIN{printf \"%.1f%%\", 100*${orphans}/${produced}}")
  fi
  echo "node 0:  produced=$produced  main-chain=$mainchain  orphans=$orphans  orphan-rate=$rate"
fi
if [ "$errs" -gt 0 ]; then
  echo "sample errors:"
