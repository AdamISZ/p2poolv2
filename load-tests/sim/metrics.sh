...
# Orphan/stale rate (reorg-immune). Count DISTINCT produced blocks vs the
# canonical chain's net growth -- NOT log-line sums. promote_block re-logs
# "Confirmed block ... references N uncle(s)" on every reorg re-promotion
# (including height=None ones), so summing those lines double-counts and
# inflates badly under forking: e.g. it reported 89% where the true orphan
# rate was ~29% (a block can reference at most MAX_UNCLES=3, yet the line-sum
# gave >10 "uncles" per main block -- proof of re-promotion double counting).
# Distinct produced = blocks node-0 ever promoted (each counted once); main =
# net confirmed-chain growth; orphans = produced - main. Equivalently this is
# 1 - main_rate/production_rate.
...
if [ -z "${chain_hi:-}" ] || [ "$produced" -eq 0 ]; then
  echo "orphan rate (node 0): n/a (no confirmed blocks yet)"
else
  mainchain=$(( chain_hi - chain_lo + 1 ))
  orphans=$(( produced - mainchain )); [ "$orphans" -lt 0 ] && orphans=0
  orate=$(awk -v o="$orphans" -v d="$produced" 'BEGIN{printf "%.1f", (d>0?100*o/d:0)}')
  echo "orphan rate (node 0): ${orate}%   ($orphans orphaned / $produced produced; main-chain length $mainchain)"
fi
